...
<div class="container-fluid py-4" style="background: #10172a; min-height: 100vh;">
    ...
    <div class="d-flex align-items-center mb-4">
        ...
        <h2 class="text-white fw-bold mb-0">Session Analysis for {{ $session->user->name }}</h2>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-xl-8 mb-4">
            ...
            <div class="card bg-dark border-0 shadow-lg rounded-4 mb-4">
                ...
                <div class="card-body">
                    <p class="text-white-50" style="white-space: pre-wrap;">{{ $session->ai_feedback }}</p>
                </div>
            </div>
            ...
            <div class="card bg-dark border-0 shadow-lg rounded-4">
                <div class="card-header bg-dark-alt border-0">
                    <h5 class="text-white fw-bold mb-0"><i class="bi bi-pencil-square me-2"></i>Manual Coach Feedback</h5>
                </div>
                <div class="card-body">
                    @if (session('success'))
                    ...
                    @endif
                    <form method="POST" action="{{ route('coach.sessions.feedback.store', $session) }}">
                        @csrf
                        <div class="mb-3">
                            <label for="manual_feedback" class="form-label text-white-50">Your Feedback</label>
                            <textarea class="form-control bg-secondary text-white border-0" id="manual_feedback" name="manual_feedback" rows="6">{{ old('manual_feedback', $session->manual_feedback) }}</textarea>
                            @error('manual_feedback')
                                <div class="text-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                        <button type="submit" class="btn btn-glow px-4">Save Feedback</button>
                    </form>
                </div>
            </div>
        </div>
